...
func bindingWithCluster(name, path, cluster string) *kcpapisv1alpha2.APIBinding {
...
func TestAuthorizationModelGeneration_Process(t *testing.T) {
	tests := []struct {
		...
		mockSetup   func(*mocks.MockManager, *mocks.MockKCPCombinedClientGetter, *mocks.MockCluster, *mocks.MockClient)
		expectError bool
	}{
		{
			name:    "error on ClusterFromContext in Process",
			binding: newApiBinding("foo", "bar"),
			mockSetup: func(manager *mocks.MockManager, allClient *mocks.MockKCPCombinedClientGetter, cluster *mocks.MockCluster, kcpClient *mocks.MockClient) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			allClientGetter := mocks.NewMockKCPCombinedClientGetter(t)
			cluster := mocks.NewMockCluster(t)
			kcpClient := mocks.NewMockClient(t)

			if test.mockSetup != nil {
				test.mockSetup(manager, allClientGetter, cluster, kcpClient)
			}

			sub := subroutine.NewAuthorizationModelGenerationSubroutine(manager, allClientGetter, "")
...
func TestAuthorizationModelGeneration_Finalize(t *testing.T) {
	tests := []struct {
		name        string
		binding     *kcpapisv1alpha2.APIBinding
		mockSetup   func(*mocks.MockManager, *mocks.MockKCPCombinedClientGetter, *kcpapisv1alpha2.APIBinding)
		expectError bool
	}{
		{
			name:    "bindings with non-matching export are skipped",
			binding: bindingWithApiExportCluster("foo", "bar", "export-cluster"),
			mockSetup: func(manager *mocks.MockManager, allClient *mocks.MockKCPCombinedClientGetter, binding *kcpapisv1alpha2.APIBinding) {
			...
			innerClient := mocks.NewMockClient(t)
			...
			allClient.EXPECT().AllClient(mock.Anything, mock.Anything).Return(innerClient, nil)
			innerClient.EXPECT().List(mock.Anything, mock.Anything).RunAndReturn(func(ctx context.Context, ol client.ObjectList, lo ...client.ListOption) error {
	...
	},
	{
		...
		mockSetup: func(manager *mocks.MockManager, allClient *mocks.MockKCPCombinedClientGetter, binding *kcpapisv1alpha2.APIBinding) {
			...
			manager.EXPECT().ClusterFromContext(mock.Anything).Return(bindingCluster, nil)
			allClient.EXPECT().AllClient(mock.Anything, mock.Anything).Return(innerClient, nil)
			innerClient.EXPECT().List(mock.Anything, mock.Anything).Return(assert.AnError)
		},
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.mockSetup != nil {
				test.mockSetup(manager, allClientGetter, test.binding)
			}
...
func TestAuthorizationModelGeneration_Finalizers(t *testing.T) {
	sub := subroutine.NewAuthorizationModelGenerationSubroutine(nil, mocks.NewMockKCPCombinedClientGetter(t), "")

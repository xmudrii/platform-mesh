...
func (mr *migrationReconciler) Reconcile(ctx context.Context, req mcreconcile.Request) (mctrl.Result, error) {
	...
	if !strings.HasPrefix(clusterName, CoordinationPrefix) {
		log.Info("Skipping migration reconciliation for non-coordination cluster", "cluster", clusterName)
		return ctrl.Result{}, nil
	}
	log = log.WithValues("cluster", clusterName).WithValues("migration", req.NamespacedName)
